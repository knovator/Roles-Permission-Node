...
const permit = async (req, res, next) => {
    if (req.route.hasOwnProperty('o')) {
        ...
        const permissionExist = await PermissionRole.countDocuments({
            permissionId: permission.id,
            roleId: { '$in': req.roleIds }
        });
...
const permission = async (id) => {
    ...
    Object.keys(permissions).map((objectKey) => {
        rolePermissions[objectKey] = lodash.map(permissions[objectKey], 'uri');
    });
    return rolePermissions;
}


const createOrUpdatePermission = async (roleId, permissions) => {
    try {
        // "permissions": [
        //     {
        //         "roleId": 3,
        ...
        //     },
        // ]
        await Permission_Role.deleteMany({ roleId: roleId });
        const result = await Permission_Role.insertMany(permissions);
        return result;
    } catch (err) {
        throw Error(err);
    }
...
const permissionRole = async (id) => {
    try {
        ...
        let permissionRoles = await PermissionRole.find({ roleId: id });
